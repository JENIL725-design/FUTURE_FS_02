...
// Always fetch fresh cart from DB to handle multi-device login
$stmt = $pdo->prepare("SELECT * FROM user_cart WHERE user_id = ?");
$stmt->execute([$user_id]);
$cart_items = $stmt->fetchAll();

// Sync current session to match DB for the checkout logic
$_SESSION['cart'] = [];
                            ...
                            <?php if (!empty($_SESSION['cart'])): ?>
                                ...
                                ?>
                                    <tr id="row-<?php echo $id; ?>">
                                        <td><div class="fw-bold"><?php echo htmlspecialchars($item['name']); ?></div></td>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center bg-light rounded-pill p-1" style="width: fit-content;">
                                                ...
                                                <span class="px-3 fw-bold" id="qty-<?php echo $id; ?>"><?php echo $item['qty']; ?></span>
                                                <button class="btn btn-sm btn-white rounded-circle qty-btn" data-id="<?php echo $id; ?>" data-change="1">
